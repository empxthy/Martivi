document.addEventListener("DOMContentLoaded", () => {
    ...
    parseButton.addEventListener('click', () => {
        ...
        if(url.includes('https://www.myhome.ge/pr/')) {
            ...
            fillUnipro(url);
            fillSS(url);
        } else if (url.includes('https://home.ss.ge/ka')) {
            fromSSfillMyHome(url);
            fromSSfillUnipro(url);
            fromSSfillSS(url);
        } else {
            alert("Invalid URL!");
        }
        ...
        chrome.tabs.query(
            ...
            (tabs) => {
                ...
                if (tabId !== undefined) {
                    chrome.tabs.sendMessage(
                        tabId,
                        { action: "ss_myhome_fill", data: data },
                        (response) => {
                            if (chrome.runtime.lastError) {
                                console.error("Error sending message to MyHome tab:", chrome.runtime.lastError.message);
                            } else {
        ...
        chrome.tabs.query(
            ...
            (tabs) => {
                ...
                if (tabId !== undefined) {
                    chrome.tabs.sendMessage(
                        tabId,
                        { action: "ss_unipro_fill", data: data },
                        (response) => {
                            if (chrome.runtime.lastError) {
                                console.error("Error sending message to Unipro tab:", chrome.runtime.lastError.message);
                            } else {
